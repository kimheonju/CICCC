    ...
    with open(filename) as f:
        ...
        for row in reader:
            evidence.append([
                ...
                MONTHS[row['Month']],  # 변환: 월을 인덱스로 변환
                int(row['OperatingSystems']),
                int(row['Browser']),
                int(row['Region']),
                int(row['TrafficType']),
                1 if row['VisitorType'] == 'Returning_Visitor' else 0,  # 변환: 방문자 유형을 숫자로 변환
                1 if row['Weekend'] == 'TRUE' else 0  # 변환: 주말 여부를 숫자로 변환
            ])
            labels.append(1 if row['Revenue'] == 'TRUE' else 0)  # 변환: 수익 여부를 숫자로 변환
